...
//     // Convert the string ID to a Mongoose ObjectId and fetch the video
//     const video = await Video.findById(new Types.ObjectId(id)).lean();
...
export async function GET(
  request: NextRequest,
  context: { params: { id: string } } // Use 'context' and access id via context.params
) {
  try {
    // Ensure the database connection is established
    await connectToDatabase();
    ...
    // Fetch the video.  No need to create a new ObjectId if already validated.
    ...
    console.error("Error fetching video:", error);
    return NextResponse.json(
      { error: "Failed to fetch video", details: error instanceof Error ? error.message : String(error) }, // Include error details
      { status: 500 }
    );
  }
}